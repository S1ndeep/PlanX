import { getCityCoordinates, getOpenTripMapKeyPreview } from "../services/opentripmap.service.js";
...
const dedupePlaces = (places = []) => {
  ...
  for (const place of places) {
    ...
    if (!seen.has(dedupeKey)) {
      seen.set(dedupeKey, {
        ...place,
        interest: inferInterestForPlace(place)
      });
    }
  }
...
const filterGeoapifyPlacesByInterests = (geoapifyPlaces, normalizedInterests) => {
  ...
  return geoapifyPlaces.filter((place) => {
    ...
    if (requestedParks && categoryText.includes("leisure.park")) {
      return true;
    }
    if (requestedMuseums && categoryText.includes("entertainment.museum")) {
      return true;
    }
    if (requestedShopping && (categoryText.includes("commercial") || categoryText.includes("shop"))) {
      return true;
    }
    if (requestedLandmarks && categoryText.includes("tourism.sights")) {
      return true;
    }
...
export const getPlacesItinerary = async (req, res) => {
  try {
    ...
    const [rawTopAttractions, geoapifyPlaces] = await Promise.all([
      getTopCityAttractions(city, Math.max(12, parsedDays * parsedPlacesPerDay)),
      getFoodAndLeisurePlaces({
        latitude: coordinates.latitude,
        longitude: coordinates.longitude,
        limit: 30
