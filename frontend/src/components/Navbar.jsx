...
const Navbar = ({ isAuthed, userName, onLogout }) => {
  ...
  return (
    <nav
      className={`navbar ${
        isHomePage
          ? "fixed inset-x-0 top-0 z-[120] pointer-events-auto transition-all duration-300"
          : "relative z-[120] pointer-events-auto border-b border-white/10 bg-transparent transition-all duration-300"
      }`}
      style={homeNavbarStyle}
    >
      <div className="container">
        <div className="navbar-inner relative z-[121] rounded-full border border-white/10 bg-[rgba(7,29,34,0.42)] px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] backdrop-blur md:px-6">
          <Link to="/" className="logo text-xl text-white sm:text-2xl">
            TripWise
          </Link>

          <div className="nav-links hidden text-white/90 lg:flex">
            {navItems.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
          ...
          <div className="hidden items-center gap-3 sm:gap-4 lg:flex">
            ...
            <button
              className="nav-cta relative z-[122] border-[#8edcff]/30 bg-[#0b3b43]/60 text-white backdrop-blur hover:border-[#53d6f7] hover:bg-[#1ec7f3] hover:text-slate-950"
        ...
        >
          <div className="mt-3 rounded-[28px] border border-white/10 bg-[rgba(7,29,34,0.9)] p-4 shadow-[0_24px_80px_rgba(2,18,24,0.2)] backdrop-blur-xl">
            ...
            <div className="mt-4 flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
              ...
              <button
                className="nav-cta relative z-[122] border-[#8edcff]/30 bg-[#0b3b43]/60 text-white hover:border-[#53d6f7] hover:bg-[#1ec7f3] hover:text-slate-950"
                onClick={handleAuthClick}
                type="button"
              >
                {isAuthed ? "Logout" : "Login"}
